...
const {checkIntentForCallTransfer, checkIntentForDtmfEntry, postRecording} = require('./utils');
...
class CallSession extends Emitter {
  constructor(logger, mrf, req, res) {
    ...
    this.locale = req.locals.locale;
    this.projectId = req.locals.projectId;
    this.hotword = req.locals.hotword || '';
    this.welcomeEvent = req.locals.welcomeEvent || 'welcome';
  }
  ...
  async exec() {
    try {
      ...
      dialog.on('destroy', () => {
        ...
        if (this.req.locals.recordpath) {
          postRecording(this.logger, this.req.locals.recordpath, this.req.locals.onRecorded, true);
        }
      });
      this.logger.info(`call connected, starting dialogflow agent ${this.projectId} using lang ${this.locale}`);

      await endpoint.set('record_stereo', true);
      const recordpath = this.req.locals.recordpath = `/tmp/${endpoint.uuid}.wav`;
      endpoint.api('uuid_record', `${endpoint.uuid} start ${recordpath}`);
      ...
      endpoint.api('dialogflow_start', `${endpoint.uuid} ${this.projectId} ${this.locale} ${this.welcomeEvent}`);
  ...
  _onIntent(ep, dlg, evt) {
    ...
    if (evt.response_id.length === 0) {
      if (this.noinput) {
        ...
        this.noinput = false;
        ep.api('dialogflow_start', `${ep.uuid} ${this.projectId} ${this.locale} actions_intent_NO_INPUT`);
      }
      else if (this.dtmfEntry) {
        ep.api('dialogflow_start', `${ep.uuid} ${this.projectId} ${this.locale} none ${this.dtmfEntry}`);
        this.dtmfEntry = null;
      }
      else {
        ep.api('dialogflow_start', `${ep.uuid} ${this.projectId} ${this.locale}`);
      }
      return;
    }
  ...
  _onTranscription(ep, evt) {
    ...
    if (this.hotword && evt.recognition_result &&
      evt.recognition_result.transcript && this.playInProgress &&
      evt.recognition_result.transcript.toLowerCase().includes(this.hotword.toLowerCase())) {

      this.logger.info(`spotted hotword ${this.hotword}, killing audio`);
