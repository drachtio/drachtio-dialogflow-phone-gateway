...
const connectCallToMs = async(req, res) => {
  ...
  try {
    ...
    dialog.on('destroy', () => {
      endpoint.destroy().catch((err) => logger.info(err, 'Error deleting endpoint'));
...
class CallSession extends Emitter {
  ...
  get ep() {
    return this.req.locals.ep;
  }

  get dlg() {
    return this.req.locals.dlg;
  }

  async exec() {
    try {
      await connectCallToMs(this.req, this.res);
      ...
      this.ep.api('dialogflow_start',
        `${this.ep.uuid} ${this.opts.project} ${this.opts.lang} ${this.opts.welcomeEvent || ''}`);
  ...
  _onIntent(ep, dlg, evt) {
    const intent = new Intent(this.logger, this.uuid, this.req, evt);
    ...
    /* collect digits? */
    else if (intent.saysCollectDtmf) {
      this.digitBuffer = new DigitBuffer(this.logger, intent.dtmfInstructions);
  ...
  _onTranscription(ep, evt) {
    const transcription = new Transcription(this.logger, this.uuid, evt);
    ...
    if (this.opts.bargePhrase && !transcription.isEmpty && this.playInProgress &&
      ...
      this.logger.info({
        phrase: this.opts.bargePhrase,
        empty: transcription.isEmpty,
  ...
  async _onAudioProvided(ep, dlg, evt) {
    ...
    if (!this.hangupAfterPlayDone && !this.callTransfer) {
      ep.api('dialogflow_start', `${ep.uuid} ${this.opts.project} ${this.opts.lang}`);
    }
    ...
    if (this.hangupAfterPlayDone) {
      ...
      try {
        ...
        this.ep.destroy().catch((err) => this.logger.info(err, 'error destroying endpoint'));
      } catch (err) {
