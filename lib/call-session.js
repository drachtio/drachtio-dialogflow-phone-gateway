...
class CallSession extends Emitter {
  ...
  async exec() {
    try {
      ...
      endpoint.api('dialogflow_start', `${endpoint.uuid} ${dfOpts.project} ${dfOpts.lang} 30 ${dfOpts.event || ''}`);
    ...
    if (this.hangupAfterPlayDone) {
      ...
      if (doRefer) {
        dlg.request({
          method: 'REFER',
          headers: {
            'Refer-To': `<sip:${ep.transferTo}@${this.req.source_address}>`,
            'Referred-By': `<sip:${this.req.callingNumber}@${this.req.source_address}>`,
