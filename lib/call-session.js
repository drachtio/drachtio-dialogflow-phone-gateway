...
class CallSession extends Emitter {
  ...
  async exec() {
    try {
      ...
      this.logger.info(`call connected, dialogflow agent ${this.dfOpts.project} using lang ${this.dfOpts.lang}`);
  ...
  _onIntent(ep, dlg, evt) {
    ...
    if (evt.response_id.length === 0) {
      if (this.noinput) {
        ...
        ep.api('dialogflow_start', `${ep.uuid} ${this.dfOpts.project} ${this.dfOpts.lang} none ${this.dtmfEntry}`);
        this.dtmfEntry = null;
      }
      else {
