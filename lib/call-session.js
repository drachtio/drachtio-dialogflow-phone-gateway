...
class CallSession extends Emitter {
  ...
  _onIntent(ep, dlg, evt) {
    ...
    // clear the no-input timer after any intent is received
  ...
  async _onAudioProvided(ep, dlg, evt) {
    ...
    if (this.hangupAfterPlayDone) {
      ...
      // every time we finish playing a prompt, start a no-input timer
      this._startNoinputTimer(ep, dlg);
    }
  }

  async _onNoInput(ep, dlg) {
    try {
      this.logger.info(`reprompting after ${config.get('dialogflow.noInputTimeout')} with no intent detected`);
      this.noinput = true;
      //ep.api('dialogflow_stop', `${ep.uuid}`);
      //await ep.api('dialogflow_start', `${ep.uuid} ${dfOpts.project} ${dfOpts.lang} 30 actions_intent_NO_INPUT`);
    } catch (err) {
      this.logger.error(err, 'Error prompting after no input');
    }
  }
