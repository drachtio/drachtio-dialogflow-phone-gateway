...
function postRecording(logger, recordpath, url, deleteAfterUpload) {
  ...
  try {
    ...
    fs
      .createReadStream(recordpath)
      .pipe(request.post(url)
        .on('error', (err) => {
          logger.error(err, `Error uploading file to ${recordpath} ${url}`);
        })
        .on('response', (res) => {
          logger.info(`upload of ${recordpath} completed with status ${res.statusCode}`);
        })
      );
  } catch (err) {
