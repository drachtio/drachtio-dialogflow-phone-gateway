...
class DigitBuffer extends Emitter {
  ...
  constructor(logger, opts) {
    super();
    this.logger = logger;
    this.minDigits = opts.min;
    this.maxDigits = opts.max || opts.min;
  ...
  process(digit) {
    ...
    if (digit === this.termDigit) {
      if (this.buffer.length >= this.minDigits) return this._fulfill();
      this.logger.debug(`discarding term digit ${digit} because we have only ${this.buffer.length} digits collected`);
    }
    this.buffer += digit;
